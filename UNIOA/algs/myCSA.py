...
class CSA_Opt(NatureOpt):
    ...
    def __call__(self):
        ...
        while not self.stop:
            ...
            # Selection
            new_X, new_X_Fit = self.Selection.improve_type(temp_X=temp_X, temp_X_Fit=temp_X_Fit, old_X=X,
                                                           old_X_Fit=X_Fit)
            # ----------------------------
            t = t + 1
            ##########################
            X_p, X_p_Fit = self.Opt_Delta_X.Personal_best(new_X=new_X, new_X_Fit=new_X_Fit, old_X_p=X_p,
                                                          old_X_p_Fit=X_p_Fit)
            X = new_X
            X_Fit = new_X_Fit
            # Y = new_Y
