...
class BA_Opt(NatureOpt):
    ...
    def __call__(self):
        ...
        while not self.stop:
            ...
            # Selection
            new_X, new_X_Fit = self.Selection.ba_type(temp_X=temp_X, temp_X_Fit=temp_X_Fit, old_X=X, old_X_Fit=X_Fit,
                                                      z2=z2)
            # ----------------------------
            t = t + 1
            new_x_g, new_x_g_fit = self.Opt_Delta_X.Global_best(new_X=new_X, new_X_Fit=new_X_Fit, old_x_g=x_g,
                                                                old_x_g_fit=x_g_fit)
